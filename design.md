...
Opaque paper background. Snowbird's supplied wordmark sits on the left; Selected work, Collage, About, and Contact sit on the right. Desktop header target is approximately 100px and mobile approximately 76px; allow growth if fonts or zoom require it. Sticky positioning is acceptable, but the header must never consume excessive screen space or cover focused content.
...
Wordmark width: approximately 224px desktop and 152px mobile using the cleaned Snowbird PNG at its original aspect ratio. At 320px it must fit beside the Menu target. The linked image uses empty alt because the link itself has the accessible name “Snowbird Landscape, home.”
...
Place one large finished-project photo underneath, nearly spanning the content width. Use a roughly 16:9 desktop frame, with an optional 16:10 frame if the photo needs more height. This is the strongest genuine after image the client supplies. Do not auto-cycle it.
...
A quiet two-column block: left image at approximately 5/12 width, right heading and two short paragraphs at 6/12 width with a column of breathing room. Heading: “About Snowbird.” A genuine approved garden detail is the current image; a portrait can replace it if the client later requests one. Never synthesize a person's likeness.
...
Finish with a full-width forest background. Within the shared container: a small “LET'S TALK” eyebrow, a large paper-colored invitation, brief supporting copy, and the verified “Call Snowbird” action. The verified Instagram and Eagle Rock location may follow. Add email only if a confirmed address is supplied.
